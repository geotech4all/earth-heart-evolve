...
CREATE POLICY "Published webinars are public" ON public.webinars
...
CREATE POLICY "Published insights are public" ON public.research_insights
  FOR SELECT USING (is_published = true);

CREATE POLICY "Admins can manage insights" ON public.research_insights
  FOR ALL USING (public.has_role(auth.uid(), 'admin'));
